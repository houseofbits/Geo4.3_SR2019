...
bool Viewport::OnInputEvent(InputEvent*const event)
{	
	//No interaction with scene
	return 1;
...
void Viewport::Render(Renderer* rnd)
{	
	glPushMatrix();
...
void Viewport::SetOrientation(float pitch, float yaw, float faar)
{
	mPitch = pitch;
	mYaw = yaw;
	mDistance = faar;
...
void Viewport::CalculateFrustumPlanes()
{
	float   proj[16];
	float   modl[16];
	float   clip[16];
...
bool	Viewport::Intersect(AxisAlignedBox& box)
{
