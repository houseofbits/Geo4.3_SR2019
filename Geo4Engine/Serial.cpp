...
LONG CSerial::Open (LPCTSTR lpszDevice, DWORD dwInQueue, DWORD dwOutQueue, bool fOverlapped)
{
	...
	if (m_hFile == INVALID_HANDLE_VALUE)
	{
		...
		cout << "Serial: Unable to open port [ERROR:" << m_lLastError <<"]"<< endl;
	...
	if (::GetDefaultCommConfig(lpszDevice,&commConfig,&dwSize))
	{
		...
		{
			// Display a warning
			cout<<"Serial: Unable to set default communication configuration"<<endl;
		}
	}
	else
	{
		// Display a warning
		cout<<"Serial: Unable to obtain default communication configuration"<<endl;
	}
	// Return successful
	return m_lLastError;
}
...
LONG CSerial::Close (void)
{
	...
	{
		// Display a warning
		cout<<"Serial: Method called when device is not open"<<endl;
		return m_lLastError;
	}
...
LONG CSerial::Setup (EBaudrate eBaudrate, EDataBits eDataBits, EParity eParity, EStopBits eStopBits)
{
	...
	if (m_hFile == 0)
	{
		...
		cout<<"Serial: Device is not opened"<<endl;
		return m_lLastError;
	}
...
CSerialEx::~CSerialEx()
{
	...
	{
		// Display a warning
		cout<<"CSerialEx::~CSerialEx - Serial port not closed"<<endl;
...
LONG CSerialEx::Open (LPCTSTR lpszDevice, DWORD dwInQueue, DWORD dwOutQueue, bool fStartListener)
{
	...
	if (m_hevtOverlappedWorkerThread == 0)
	{
		// Obtain the error information
		m_lLastError = ::GetLastError();
		cout<<"CSerialEx::Open - Unable to create event"<<endl;;
...
LONG CSerialEx::StartListener (void)
{
	...
	if (m_hThread == 0)
	{
		...
		{
			// Display a warning
			cout<<"CSerialEx::StartListener - Unable to start COMM watcher thread"<<endl;
...
DWORD CSerialEx::ThreadProc (void)
{
	...
	do
	{
		...
		//cout<<"fck "<<m_fStopping<<endl;
		if(m_fStopping == true)break;
		...
		{	
			...
			// Issue an error and quit
			cout<<"CSerialEx::ThreadProc - Unable to wait until COM event has arrived"<<endl;
			return m_lLastError;
		}
//		cout<<"fck2"<<endl;
#else
		...
		if (!m_fStopping)
		{
			...
			if (!::ClearCommError(m_hFile,&dwErrors,0))
			{
				...
				// Issue an error and quit
				cout<<"CSerialEx::ThreadProc - Unable to obtain COM status"<<endl;
			}
